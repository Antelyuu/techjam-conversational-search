...
MEASURED (P7-T2, seed 20260831, BM25 route, this catalogue) ON THE
phase/7-paraphrase-robustness RANKING CODE. Every score and overlap figure in
this docstring -- this table and the --keep-carrier split below -- was carried
over verbatim when the mode was ported here and has NOT been re-measured
against this branch's reranker, so read them as the shape of the two
mechanisms rather than as this branch's numbers. The `--audit` table further
down does not touch the reranker and does reproduce here.
...
#
# WHY A SECOND ONE AT ALL. The synonym paraphraser above is a 73-entry
# hand-built lexicon -- `len(SYNONYMS)`, counted rather than estimated; the
# P7 original said "~130" here and no count in the table reproduces that.
# Four ranking changes tuned against it would be tuned
